...
func TestArmsInterleavedWithPlainFields(t *testing.T) {
	...
	one := &buffers.Oneof{Node: "m.p", Name: "p", UnionName: "MP"}
	...
	m := &buffers.Message{
		Name:   "M",
		Fields: []*buffers.Field{field("a", 1, 0), x, b, y},
...
func TestSkippedOneofHoldsOneId(t *testing.T) {
	one := &buffers.Oneof{Node: "m.p", Name: "p", UnionName: "MP", Skip: true}
	x := field("x", 2, 1)
	x.Oneof = one
	one.Fields = []*buffers.Field{x}

	m := &buffers.Message{
		Name:   "M",
		Fields: []*buffers.Field{field("a", 1, 0), x, field("c", 3, 2)},
		Oneofs: []*buffers.Oneof{one},
	}
...
func TestUnionArmsAreOrderedByOrdinal(t *testing.T) {
	...
	one := &buffers.Oneof{Node: "m.p", Name: "p"}
	late, early := field("late", 9, 8), field("early", 2, 1)
	one.Fields = []*buffers.Field{late, early}
...
func TestUnionArmsSkipExcluded(t *testing.T) {
	...
	drop.Skip = true
	one.Fields = []*buffers.Field{keep, drop}
...
func armNames(fs []*buffers.Field) []string {
